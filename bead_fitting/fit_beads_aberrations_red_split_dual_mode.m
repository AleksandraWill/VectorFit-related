...
path_input_data = 'C:\Users\Aleksandra\Libraries\vectorfit\data\beads_red_blue_split_red_dual\avg_pos2_red_blue_split.tif';

% Folder where the results will be stored
path_output_data = 'C:\Users\Aleksandra\Libraries\vectorfit\data\beads_red_blue_split_red_dual\';
params = set_parameters_TetraSpeck_beads_100_red_split_dual_mode;
...
for file_i=1:nr_of_files
    ...
    % --- Integrate image loading and cropping here ---
    info = imfinfo(path_input_data_full);
    num_frames = numel(info);
    img = zeros(info(1).Height, info(1).Width, num_frames, 'like', imread(path_input_data_full, 1));
    for k = 1:num_frames
        img(:,:,k) = imread(path_input_data_full, k);
    end

    % Crop to upper half
    img_cropped = img(257:512, :, :); % bottom 256 rows, all columns, all frames

    % Update parameters to match cropped image size
    params.imgSizeY = size(img_cropped, 1);
    params.imgSizeX = size(img_cropped, 2);

    % Use img_cropped in all subsequent analysis
    allframes = img_cropped;

    % -------------------------------------------------

    % Segmentation
    segmentation_beads_difference_uniform_cropped
